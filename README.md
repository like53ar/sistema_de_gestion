...
- **⚙️ Parámetros Generales**: Padrón centralizado de datos maestros del sistema, incluyendo tablas oficiales de **Provincias**, **Países**, **Bancos**, **Monedas**, **Códigos de IVA**, **Tipos de Comprobantes** y **Tipos de Documento**. Además, incorpora validaciones dinámicas con formato inteligente para códigos alfanuméricos. Junto con esto, cuenta con la sección de **Parámetros de Módulos** (*Ventas, Compras, Tesorería, Contabilidad, Sueldos y Stock*) con ventanas emergentes (Pop-ups LG/XL). Se ha implementado una **Arquitectura de Navegación por Árbol Jerárquico** para los módulos de Compras, Tesorería, Contabilidad, Sueldos y Stock, permitiendo navegación multinivel con indicadores de expansión `[+]` y `[-]`. El módulo de **Ventas** destaca por su configuración ultra-granular mediante pestañas, mientras que **Compras** presenta un árbol detallado con secciones de *Archivos (Actualizaciones, Parametrización Contable, Carga Inicial)*, *Solicitudes*, *Órdenes*, *Comprobantes*, *Cuentas Corrientes* y *Análisis Multidimensional*.
...
- **🛒 Compras**: Control de órdenes de compra, registro de facturas y recepción de mercadería. **Gestión Avanzada de Proveedores** con un árbol de configuración multidimensional que incluye: validación algorítmica de CBU (BCRA), gestión financiera (pagos, cheques, echeq), árbol dinámico de clasificación por familia/grupo/individuo, y un maestro de artículos/conceptos con filtrado en tiempo real.
